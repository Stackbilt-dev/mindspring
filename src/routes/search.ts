 ...
 *
 * Uses a broad search with zero threshold as a browse mechanism.
 * Query params:
...
search.get('/conversations', async (c) => {
  ...
  // Qdrant scroll for pagination without a query vector
  const qdrant = new QdrantClient(c.env)
  const offset = c.req.query('offset') ?? undefined

  const url = `${c.env.QDRANT_CLOUD_URL}/collections/${c.env.QDRANT_COLLECTION}/points/scroll`
...
search.get('/conversations/:id/similar', async (c) => {
  ...
  // Fetch the conversation's text to use as query
  const url = `${c.env.QDRANT_CLOUD_URL}/collections/${c.env.QDRANT_COLLECTION}/points/scroll`
  const scrollResponse = await fetch(url, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'api-key': c.env.QDRANT_API_KEY,
    ...
    body: JSON.stringify({
      filter: { must: [{ key: 'id', match: { value: conversationId } }] },
      limit: 1,
      with_payload: true,
    }),
  })
  ...
  const text = scrollData.result.points[0].payload.text as string
